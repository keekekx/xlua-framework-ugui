...
function _M:Fire(...)
    ...
    if self.listener then
        print("insert ...args")
        table.insert(self.args, 1, self.listener)
    end
    print("><>>>>>_M:Fire args count:" .. #self.args)
    -- print_r(self.listener)
    self.executing = true
    --[[
    if self.waiter then
        self.waiter = false
    ...
    for k, v in pairs(self.handlers) do
        ...
        spawn(function()
            -- print(tostring(v))
            v(table.unpack(self.args))  --lua 5.2调用unpack需要table.unpack 调用
            i = i - 1
            if i == 0 then
...
function _M:Wait()
    ...
    while self.waiter or not self.args do
        if wait then
            wait()
        end
    end
...
if false then
    ...
    if failhorribly then
        ...
        e:connect(function()
            e:WaitForCompletion()
            print 'done with connected function'
        end)
        e:Fire()
        print 'done'
    end
